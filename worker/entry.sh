...
rm -rf $GOBBLIN_HOME/conf/standalone/worker/application.conf
rm -rf $GOBBLIN_HOME/conf/standalone/worker/reference.conf

# Multiple Replacement, the / character in the environment variable was not working, the delimited was changed to | from \
sed -e "s|\${JOB_CONFIG_PATH}|$JOB_CONFIG_PATH|;s|\${ZOOKEEPER_CONNECT_URL}|$ZOOKEEPER_CONNECT_URL|;s|\${KAFKA_BROKER_URL}|$KAFKA_BROKER_URL|;s|\${HELIX_CLUSTER_NAME}|$HELIX_CLUSTER_NAME|;s|\${KAFKA_TOPIC}|$KAFKA_TOPIC|" ./support/templates/application.tpl >> $GOBBLIN_HOME/conf/standalone/worker/application.conf

sed -e "s|\${ZOOKEEPER_CONNECT_URL}|$ZOOKEEPER_CONNECT_URL|;s|\${HELIX_CLUSTER_NAME}|$HELIX_CLUSTER_NAME|;s|\${HDFS_HOST_PORT}|$HDFS_HOST_PORT|" ./support/templates/reference.tpl >> $GOBBLIN_HOME/conf/standalone/worker/reference.conf

# Move the logging file too
cp -f ./support/templates/log4j-cluster.properties $GOBBLIN_HOME/conf/standalone/worker

chmod +x ./bin/**
sleep 1;
./bin/gobblin-cluster-worker.sh start
